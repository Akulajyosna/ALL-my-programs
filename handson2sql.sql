...
select  count(*)  as counting from salesman
select * from salesman

drop table orders
create table orders(ord_no int primary key,purch_amt decimal(8,2),ord_date Date,customer_id int,salesman_id int references salesman(salesman_id))
...
select ord_date,salesman_id,count(*)as counting from orders group by ord_date,salesman_id
select count(*) as counting from orders  where ord_date='2012-08-17'
select salesman_id,max(purch_amt) as maximum from orders group by salesman_id having salesman_id BETWEEN 5003 AND 5008
select * from orders
