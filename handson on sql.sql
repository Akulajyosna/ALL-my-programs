...
select AVG (purch_amt) as averageamount from orders
select max(purch_amt) as maxamount from orders
select customer_id,max(purch_amt) as high from orders group by customer_id
...
select COUNT(grade) as 'grades' from  customers;
select city,MAX(grade) as high from customers group by city;
